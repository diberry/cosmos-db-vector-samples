        ...
        for (const hotel of batch) {
            const insertSQL = `
                INSERT INTO ${config.tableName} (
                    ...
                    parkinc_included, last_renovation_date, rating,
 ...
 */
async function createIVFFlatIndex(client: any): Promise<void> {
    console.log(`\nCreating IVFFlat index with ${config.lists} lists...`);
    
    const createIndexSQL = `
        CREATE INDEX ON ${config.tableName} 
        USING ivfflat (${config.embeddedField} vector_cosine_ops) 
...
async function main() {
    ...
    try {
        ...
        await createIVFFlatIndex(client);
        ...
        // Print the results
        printSearchResults(searchResults, config.query, 'IVFFlat');
